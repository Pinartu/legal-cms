...
export default async function SearchPage({ params, searchParams: sp }: { params: Promise<{ lang: string }>; searchParams: Promise<{ q?: string }> }) {
  ...
  const posts = await prisma.post.findMany({
    where: {
      isPublished: true,
      locale: lang,
      ...(q ? { OR: [{ title: { contains: q, mode: 'insensitive' as const } }, { content: { contains: q, mode: 'insensitive' as const } }, { metaDescription: { contains: q, mode: 'insensitive' as const } }] } : {}),
    },
  ...
  async function searchAction(formData: FormData) {
    ...
    if (query) redirect(`/${l}/search?q=${encodeURIComponent(query)}`);
    else redirect(`/${l}/search`);
  }
    ...
    <div className="bg-white min-h-screen">
      <div className="bg-[#1a2332] pt-36 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <form action={searchAction} className="relative flex items-center max-w-2xl mx-auto">
            ...
            <input type="text" name="q" defaultValue={q} placeholder={t(locale, 'page.search_placeholder')} className="w-full pl-14 pr-32 py-4 bg-white/10 border border-white/20 text-white placeholder:text-white/30 focus:border-[#b8860b] focus:outline-none transition-colors text-base" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="mb-10 pb-4 border-b border-zinc-200">
          {q ? (
            <h2 className="text-lg text-zinc-600"><span className="font-semibold text-[#1a2332]">&ldquo;{q}&rdquo;</span> {t(locale, 'page.results_for')} {posts.length} {t(locale, 'page.results_found')}</h2>
          ) : (
            <h2 className="text-lg text-zinc-600">{locale === 'tr' ? 'Tüm Yayınlar' : 'All Publications'} <span className="text-zinc-400">({posts.length})</span></h2>
          )}
        </div>
        {posts.length > 0 ? (
          ...
          <div className="text-center py-20 border border-dashed border-zinc-300">
            ...
            <p className="text-zinc-400 text-sm">{q ? t(locale, 'page.no_results_hint') : (locale === 'tr' ? 'Henüz yayınlanmış yazı bulunmuyor.' : 'No published posts yet.')}</p>
          </div>
        )}
      </div>
    </div>
