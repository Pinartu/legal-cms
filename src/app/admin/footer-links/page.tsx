...
const COLUMNS = [
  { value: 'explore', label: 'Keşfedin / Explore' },
  { value: 'publications', label: 'Yayınlar / Publications' },
  { value: 'legal', label: 'Yasal / Legal' },
];
...
export default function FooterLinksPage() {
  ...
  const [form, setForm] = useState({ label: '', url: '', column: 'explore', locale: 'tr', order: 0, openInNewTab: false });
  ...
  const resetForm = () => {
    setForm({ label: '', url: '', column: 'explore', locale: activeLocale, order: 0, openInNewTab: false });
    setEditingId(null);
    setShowForm(false);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await fetch(`/api/footer-links/${editingId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(form),
        });
      } else {
        await fetch('/api/footer-links', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(form),
        });
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/footer-links/${id}`, { method: 'DELETE' });
    await fetchLinks();
  };

  const handleEdit = (link: FooterLink) => {
    setForm({ label: link.label, url: link.url, column: link.column, locale: link.locale, order: link.order, openInNewTab: link.openInNewTab });
    setEditingId(link.id);
    setShowForm(true);
  };

  // Filter links by active locale
  const filteredLinks = links.filter(l => l.locale === activeLocale);
  const groupedLinks = COLUMNS.map(col => ({
    ...col,
    links: filteredLinks.filter(l => l.column === col.value).sort((a, b) => a.order - b.order),
  }));
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-3xl font-serif font-bold text-slate-900">Footer Linkleri</h2>
            <p className="text-sm text-slate-500 mt-1">Footer&apos;daki navigasyon linklerini dil bazında yönetin</p>
          </div>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white border border-slate-200 rounded-lg p-6 mb-8 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Kolon</label>
              <select value={form.column} onChange={e => setForm({ ...form, column: e.target.value })} className="w-full p-3 border border-slate-300 rounded-lg text-sm">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Sıra</label>
      ...
      {/* Links grouped by column */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {groupedLinks.map(group => (
          <div key={group.value} className="bg-white border border-slate-200 rounded-lg overflow-hidden">
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-200">
              <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">{group.label}</h3>
            </div>
            <div className="p-2">
              {group.links.length === 0 ? (
                <p className="text-sm text-slate-400 text-center py-4">Henüz link eklenmemiş</p>
              ) : (
                <ul className="space-y-1">
                  {group.links.map(link => (
                    <li key={link.id} className="flex items-center justify-between p-2 rounded hover:bg-slate-50 group">
                      <div className="flex items-center gap-2 min-w-0">
                        <GripVertical className="w-4 h-4 text-slate-300 flex-shrink-0" />
                        <div className="min-w-0">
                          <span className="text-sm text-slate-700 block truncate">{link.label}</span>
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                        <button onClick={() => handleEdit(link)} className="p-1 text-slate-400 hover:text-slate-700"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                        <button onClick={() => handleDelete(link.id)} className="p-1 text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </li>
                  ))}
                </ul>
